...
fn pgcache_cmd(dbs: &TempDBs, listen_port: u16, metrics_port: u16, extra_args: &[&str]) -> Command {
    ...
    // Disable per-connection read-after-write (PGC-124) by default via its
    // fault-injection hook (a no-op on a non-fault build): nearly every
    ...
    cmd.env("PGCACHE_FAULT_READ_YOUR_WRITES_OFF", "1");
...
fn pgcache_spawn(
    ...
    let child = pgcache_cmd(dbs, listen_port, metrics_port, extra_args)
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()
        .expect("run pgcache");
...
/// Spawn pgcache with extra environment variables (e.g. fault-injection arming).
/// Env is set on the child process only, so it cannot leak to parallel tests.
fn pgcache_spawn_env(
    dbs: &TempDBs,
    listen_port: u16,
    metrics_port: u16,
    extra_args: &[&str],
    ...
    let mut cmd = pgcache_cmd(dbs, listen_port, metrics_port, extra_args);
