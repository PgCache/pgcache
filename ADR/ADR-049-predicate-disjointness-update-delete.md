...
## Rationale
- **One primitive, three kinds.** Predicate disjointness generalizes INSERT's point check, so the same `column_ranges_disjoint` serves all three and future precision (multi-column, `IN`/`BETWEEN`) lands in one place.
...
### Negative
- Precision is single-column equality/range, AND-only, literal values; multi-column predicates, `IN`/`BETWEEN`, and bind-value substitution for UPDATE/DELETE are follow-ups.
