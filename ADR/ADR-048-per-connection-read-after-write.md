...
- **Bounded state.** Writes aggregate per table (a bulk load collapses to O(1)); a two-segment active/waiting pair recovers LSN granularity so a fresh write never gates an already-applied one.
